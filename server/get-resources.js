...
module.exports = get = async () => {
  ...
  const urls_path = resources.map(({ url, slug }) => ({
    url,
    w800: path.join(process.cwd(), "static", "screenshots", `${slug}.800.png`), // add process.cwd() for g-image
    w400: path.join(process.cwd(), "static", "screenshots", `${slug}.400.png`),
    w200: path.join(process.cwd(), "static", "screenshots", `${slug}.200.png`)
  }));
  ...
  resources = resources.map(resource => {
    ...
    return {
      ...
      screenshot: {
        ...
        w200: screenshot ? screenshot.w200 : null,
      }
    };
  });
